...
const OptionColums = ({ product }) => {
  ...
  const handleMutation = (mutationOptions, successMessage, errorMessage) => {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          goToToast(successMessage, staticServerUri + "/cart", navigate);
        },
        onError: () => {
  ...
  return (
    <div className="option-columns ml-8 mt-8 sm:text-sm md:text-base text-gray-800 w-[400px]">
      ...
      {selectedOptions.map((option) => (
        <ol
          key={option.id}
          className="seleted-option-list bg-gray-50 p-4 relative mt-4 mb-3"
        >
          <span className="name text-sm block w-10/12 whitespace-pre-wrap">
            {option.name}
          </span>
          <button
